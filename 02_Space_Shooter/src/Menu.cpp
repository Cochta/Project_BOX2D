...
void Menu::Main()
{
	...
	rules.setPosition(sf::Vector2f(Properties::WINDOW_SIZE_WIDTH / 2, Properties::WINDOW_SIZE_HEIGHT - Properties::WINDOW_SIZE_HEIGHT / 2));
...
void Menu::InGame(int HP, int score)
{
	_elements.clear();
	_texts.clear();
	...
	healthText.setString("SCORE: " + std::to_string(score) + "	Lives left: " + std::to_string(HP));
	healthText.setColor(sf::Color::White);
...
void Menu::GameOver(int score)
{
	...
	end.setSize(sf::Vector2f(Properties::WINDOW_SIZE_WIDTH / 2, Properties::WINDOW_SIZE_HEIGHT / 1.5));
	end.setFillColor(sf::Color::Black);
	end.setOrigin(end.getSize() * 0.5f);
	end.setPosition(sf::Vector2f(Properties::WINDOW_SIZE_WIDTH / 2, Properties::WINDOW_SIZE_HEIGHT / 2));
...
void Menu::Rules()
{
	...
	rules.setSize(sf::Vector2f(Properties::WINDOW_SIZE_WIDTH / 1.5, Properties::WINDOW_SIZE_HEIGHT / 1.5));
	rules.setFillColor(sf::Color::Black);
	rules.setOrigin(rules.getSize() * 0.5f);
	rules.setPosition(sf::Vector2f(Properties::WINDOW_SIZE_WIDTH / 2, Properties::WINDOW_SIZE_HEIGHT / 2));
